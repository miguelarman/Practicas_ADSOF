	...
	*/
	public long combinaciones(int n, int k){
		...
		//caso general
			...
			if (cache.containsKey(posicion(n, k))){ //Si tenemos la clave ya guardada en la cache
				return(cache.get(posicion(n, k)));	//La extraemos y la devolvemos
			}
			else{	//Si aun no la tenemos guardada
				num =  combinaciones(n-1, k-1)+ combinaciones (n-1, k);	//La calculamos con recursividad
				cache.put(posicion(n, k), num);	//La metemos en la cache
				cache.put(posicion(n, n - k), num);	//Metemos el numero combinatorio n sobre (n - k), ya que sabemos que tiene el mismo valor
				return num; //Regresamos el valor calculado
			}
